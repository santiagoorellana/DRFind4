...
type
  ...
  TWaveAudioGetFormatEvent = procedure(Sender: TObject; out pWaveFormat: PWaveFormatEx; var FreeIt: Boolean) of object;
  TWaveAudioGetDataEvent = function(Sender: TObject; const Buffer: Pointer; BufferSize: DWORD; var NumLoops: DWORD): DWORD of object;
  TWaveAudioGetDataPtrEvent = function(Sender: TObject; out Buffer: Pointer; var NumLoops: DWORD; var FreeIt: Boolean): DWORD of object;
  ...
  TWaveAudioFilterEvent = procedure(Sender: TObject; const Buffer: Pointer; BufferSize: DWORD) of object;
...
function GetWaveAudioInfo(mmIO: HMMIO; out pWaveFormat: PWaveFormatEx; out DataSize, DataOffset: DWORD): Boolean;

// Initializes a new wave RIFF format in an open mmIO handle. The previous
// content of mmIO will be lost.
function CreateWaveAudio(mmIO: HMMIO; const pWaveFormat: PWaveFormatEx; out ckRIFF, ckData: TMMCKInfo): Boolean;
...
function GetStreamWaveAudioInfo(Stream: TStream; out pWaveFormat: PWaveFormatEx; out DataSize, DataOffset: DWORD): Boolean;
...
function CreateStreamWaveAudio(Stream: TStream; const pWaveFormat: PWaveFormatEx; out ckRIFF, ckData: TMMCKInfo): HMMIO;
...
function GetWaveAudioPeakLevel(const Data: Pointer; DataSize: DWORD; BitsPerSample: WORD): Integer;

// Inverts the wave data (PCM format only).
procedure InvertWaveAudio(const Data: Pointer; DataSize: DWORD; BitsPerSample: WORD);

// Fills the wave data with silence
procedure SilenceWaveAudio(const Data: Pointer; DataSize: DWORD; BitsPerSample: WORD);

// Increases/Decreases the wave data volume by the specified percentage (PCM format only).
procedure ChangeWaveAudioVolume(const Data: Pointer; DataSize: DWORD; BitsPerSample: WORD; Percent: Integer);
...
{ A subset of Microsoft Audio Compression Manager (ACM) API }
{ http://msdn.microsoft.com/library/en-us/multimed/htm/_win32_audio_compression_manager.asp }
...
begin
case Indice of
     0: Result := Stereo16bit8000Hz;
     1: Result := Stereo16bit11025Hz;
     2: Result := Stereo16bit22050Hz;
     3: Result := Stereo16bit44100Hz;
     4: Result := Stereo16bit48000Hz;
     else Result := Stereo16bit44100Hz;
     end;
end;
