...
unit UAudioEntradaBasico;

interface

uses Windows, Messages, Classes, mmSystem, UAudioUtiles, UAudioIO;

type

// Clase base abstrapta para componentes de captura de audio.
TWaveAudioIn = class(TWaveAudioIO)
  ...
  protected
    ...
    class function GetNumDevs: DWORD; override;
    ...
    function AddBuffer(const Buffer: Pointer; BufferSize: DWORD; FreeIt: Boolean): Boolean; virtual;
    procedure GetWaveFormat(out pWaveFormat: PWaveFormatEx; var FreeIt: Boolean); override;
    procedure DatosListos(const Buffer: Pointer; BufferSize: DWORD; var FreeIt: Boolean); virtual;
  protected
...
//-----------------------------------------------------------------------------
class function TWaveAudioIn.GetNumDevs: DWORD;
begin
...
// Devuelve la posición de la grabación en milisegundos.
...
// Obtiene la cadena con el mensaje de error.
...
// Establece el formato de grabación.
...
// Devuelve True si el ID de dispositivo indicado es válido.
   ...
   if not Active then
      ...
      try
         ...
         try
            ...
            if FreeWaveFormat then FreeMem(pWaveFormat);
         end;
      finally
         Opening := False;
         Unlock;
      ...
      raise EWaveAudioInvalidOperation.Create('El dispositivo ya se encuentra abierto.');
   end;
end;
...
if not Closing then
   ...
   if Active then
      ...
      raise EWaveAudioInvalidOperation.Create('El dispositivo ya se encuentra cerrado.');
   end;
end;

// Generar una pausa en la grabación.
...
// Sale de una pausa para continuar grabando.
...
//-----------------------------------------------------------------------------
function TWaveAudioIn.AddBuffer(const Buffer: Pointer; BufferSize: DWORD; FreeIt: Boolean): Boolean;
var pWaveHeader: PWaveHdr;
begin
Result := False;
pWaveHeader := nil;
if ReallocateBuffer(pWaveHeader, BufferSize, Buffer) then
   begin
   if FreeIt then pWaveHeader^.dwUser := DWORD(Self);
   try
...
// Devuelve el formato de audio de la grabación.
procedure TWaveAudioIn.GetWaveFormat(out pWaveFormat: PWaveFormatEx; var FreeIt: Boolean);
begin
if PCMFormat <> nonePCM then
   ...
   if Assigned(fOnFormat) then fOnFormat(Self, pWaveFormat, FreeIt);
end;

//-----------------------------------------------------------------------------
procedure TWaveAudioIn.DatosListos(const Buffer: Pointer; BufferSize: DWORD; var FreeIt: Boolean);
begin
//Este método debe ser sobrescrito en la clase decendiente para recibir el buffer con los datos.
end;
...
try
   try
      if Assigned(pWaveHeader) and
         ...
         DatosListos(pWaveHeader^.lpData, pWaveHeader^.dwBytesRecorded, FreeBuffer);
      ...
      if Closing or (ActiveBufferCount > BufferCount) or
         ...
         not AddWaveHeader(pWaveHeader) then
         ReallocateBuffer(pWaveHeader, 0, nil);
   except
